...
Map::Map(int map_width_, int map_height_, vector<string> map, shared_ptr<TextureManager> texture_manager_, shared_ptr<SoundManager> sound_manager_,
    ...
    for (int i = 0; i < map_height; i++)
        for (int j = 0; j < map_width; j++) {
            ...
            if (map[i][j] == PATH || map[i][j] == START || map[i][j] == FINISH) {
                ...
                if (map[i][j] == FINISH) finish_point = Vector2f(j * TILE_SIZE + TILE_SIZE / 2, i * TILE_SIZE + TILE_SIZE / 2);
...
void Map::constructNormal(Vector2f position, Vector2i start_v_dir) {
    ...
    shared_ptr<Balloon> new_balloon =
        make_shared<Normal>(texture, position, start_v_dir, BALLOON_SPEED, POINT, frozen_texture, popped_texture, sound_manager);
...
void Map::constructPregnant(Vector2f position, Vector2i start_v_dir) {
    ...
    shared_ptr<Balloon> new_balloon = make_shared<Pregnant>(texture, position, start_v_dir, BALLOON_SPEED, POINT, frozen_texture, sound_manager);
    balloons.push_back(new_balloon);
}
        ...
        if (balloon->getState() == DESTROYED) {
            ...
            if (balloon->getType() == PREGNANT) {
                if (balloon->getVDir().x == 0) {
                    float first_x = balloon->getPosition().x, first_y = balloon->getPosition().y - generateRandom(15, 20);
                    float second_x = first_x, second_y = balloon->getPosition().y + generateRandom(15, 20);
                    ...
                    int first_x = balloon->getPosition().x - generateRandom(15, 20), first_y = balloon->getPosition().y;
                    int second_x = balloon->getPosition().x + generateRandom(15, 20), second_y = first_y;
                    spawnNormalBalloon(Vector2f(first_x, first_y), balloon->getVDir());
                    spawnNormalBalloon(Vector2f(second_x, second_y), balloon->getVDir());
                }
            }
...
void Map::spawnNormalBalloon(Vector2f pos, Vector2i dir) {
    ...
    shared_ptr<Balloon> normalBalloon = make_shared<Normal>(texture, pos, dir, BALLOON_SPEED, POINT, frozen_texture, popped_texture, sound_manager);
        ...
        if (tile->contains(mouspos)) {
            if (tile->canPlantTower() == true) {
                ...
                if (tower->getName() == NORMAL_SHOOTER) {
                    ...
                    new_tower = make_shared<FireTower>(
                        position, tower->getPrice(), tower->getCoolDownTime(), tower->getTexture(), tower->getRadius(), ray_texture, sound_manager);
                }

                if (tower->getName() == ICE_SHOOTER) {
                    Texture &ray_texture = texture_manager->getTexture(ICE_RAY_FILENAME);
                    new_tower = make_shared<IceTower>(
                        position, tower->getPrice(), tower->getCoolDownTime(), tower->getTexture(), tower->getRadius(), ray_texture, sound_manager);
                }

                if (tower->getName() == CANNON) {
                    Texture &ray_texture = texture_manager->getTexture(CANNON_RAY_FILENAME);
                    Texture &explosion_texture = texture_manager->getTexture(EXPLOSION_IMAGE_FILENAME);
                    new_tower = make_shared<Cannon>(position, tower->getPrice(), tower->getCoolDownTime(), tower->getTexture(), tower->getRadius(),
