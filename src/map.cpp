...
void Map::drawBalloons(RenderWindow &window, float dt)
{
    for (auto it = balloons.begin(); it != balloons.end();it++)
    {
        ...
        if (balloon->isDestroyed())
        {
            ...
            {
                Vector2i v_dir = balloon->getVDir();
                if (v_dir.y == 0)
                {
                    constructNormal(Vector2f(balloon->getPosition().x, balloon->getPosition().y), v_dir);
                    constructNormal(Vector2f(balloon->getPosition().x + 10, balloon->getPosition().y), v_dir);
                }
                else if (v_dir.x == 0)
                {
                    constructNormal(Vector2f(balloon->getPosition().x, balloon->getPosition().y), v_dir);
                    constructNormal(Vector2f(balloon->getPosition().x, balloon->getPosition().y + 10), v_dir);
                }
            }
    ...
    balloons.erase(remove_if(balloons.begin(), balloons.end(), [](shared_ptr<Balloon> n)
